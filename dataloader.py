...
import torch
import numpy as np
from utils import encode_label, encode_meta_label,encode_meta_choosed_label
...
class SkinDataset(data.Dataset):
    def __init__(self, image_dir, img_info, file_list, shape, is_test=False, num_class=1, type="clic"):
    ...
    def __getitem__(self, index):
        ...
        # meta_data, w1, w2 = encode_meta_label(sub_img_info, file_id)

        #metadata-features choose here!
        meta_data_one_hot, meta_data = encode_meta_choosed_label(sub_img_info, file_id)
        meta_data_one_hot = torch.tensor(np.array([meta_data_one_hot]))

        meta_data = torch.tensor(np.array([meta_data]))

        if self.type == "clic":
            return clinic_img, [total_label[0], total_label[1], total_label[2], total_label[3],
                                total_label[4], total_label[5], total_label[6], total_label[7]]
        elif self.type == "derm":
            return dermoscopy_img, [total_label[0], total_label[1], total_label[2], total_label[3],
                                    total_label[4], total_label[5], total_label[6], total_label[7]]
        else:
            return clinic_img, dermoscopy_img, meta_data_one_hot, [total_label[0], total_label[1], total_label[2],
                                                           total_label[3],
                                                           total_label[4], total_label[5], total_label[6],
                                                           total_label[7]]
...
def generate_dataloader(shape, batch_size, num_workers, data_mode, type):
    test_index_df = pd.read_csv(test_index_path)
    train_index_df = pd.read_csv(train_index_path)
    val_index_df = pd.read_csv(val_index_path)
